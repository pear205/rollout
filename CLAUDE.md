# 오늘도 출동 (Roll Out)

영유아를 데리고 외출하는 부모를 위한 준비물 앱.
월령·외출 시간·장소·기온·수유 방식을 입력하면, 오늘 챙길 것을 **수량까지 계산해서** 알려준다.

- **부제 / 앱스토어 설명 첫 줄:** 영유아 외출 준비물 — 오늘 챙길 것만 딱
- **주 대상:** 0~3세 아이의 부모 (엄마·아빠 모두)
- **사용 순간:** 폰으로, 현관을 나서기 직전. 급하다. 입력이 길면 안 쓴다.

## 핵심 차별점

기존 육아 체크리스트 앱은 전부 **고정 목록**이다.
이 앱은 **조건에 따라 항목과 수량이 달라진다.**
"3시간 외출이면 기저귀 3장" 같은 계산이 이 앱의 존재 이유이므로,
기능을 추가할 때 이 차별점을 흐리지 않는지 항상 확인할 것.

---

## 현재 상태 (2026-07 기준)

### 배포된 주소 / 계정

- 앱: `https://pear205.github.io/rollout/`
- 저장소: `https://github.com/pear205/rollout` (public)
- Supabase project-ref: `nzqydlttqrdyrtdgjmbg`
- OAuth 콜백: `https://nzqydlttqrdyrtdgjmbg.supabase.co/auth/v1/callback`

### 완료
- `index.html` — 단일 파일. 빌드 도구 없음. 그대로 열면 동작한다.
  - 규칙 엔진 (아이템 41개), 조건 입력 폼, 체크리스트, 진행 게이지
  - 임무(프리셋) 5개 하드코딩
  - URL 해시로 조건 공유 (`#k=` + base64)
- Supabase 프로젝트 생성 완료 (리전: Northeast Asia / Seoul, Free 플랜)
- `schema.sql` 적용 완료 — 테이블 3개, 트리거 3개, RLS 정책 10개, GRANT, 뷰 1개
  - 검증됨: 세 테이블 모두 `rowsecurity = true`, `pg_policies` 10행
- **GitHub Pages 배포** 완료
- **Google OAuth** 완료 — Google Cloud Console 클라이언트 발급 → Supabase Provider 등록 → URL Configuration까지. 실제 로그인 동작 확인됨
- **`Store` → Supabase 전환** 완료 — 로그인 시 `profiles.settings`(jsonb), 비로그인 시 localStorage
- **로그인 UI** 완료 — 구글 버튼 / 닉네임 표시 / 로그아웃
- **임무 저장·불러오기** 완료 — `kits` 테이블. `S.ageOverride`로 저장 시점 월령을 고정한다
- **수량 구조화** 완료 — `q`가 `{value, unit}`을 반환. 표시는 `fmtQty()`가 담당
- **저장 디바운스** 완료 — localStorage는 즉시, 클라우드는 1200ms 디바운스 + `pagehide`/`visibilitychange`/로그아웃 직전 flush
- **PWA** 완료 — `manifest.json`, `sw.js`, `icons/`. 배포 환경에서 등록·오프라인 캐시·Supabase 요청 비캐싱까지 검증됨
- **공개·별점** 완료 — 내 임무를 "공개하기"로 전환(비가역, 되돌리는 기능 없음) → `kits_with_author` 뷰가 `PRESETS` 자리를 대체.
  공개 임무 0개면 여전히 `PRESETS`로 폴백(콜드스타트). 본인 킷은 별점 입력이 안 뜨고, RLS도 이중으로 막는다(직접 우회 시도로 검증됨)
- **가방 추천** 완료 — 항목별 `vol`(부피 등급) 합계 → `BAG_TIERS`가 4단계 문구로 매핑. 게이지 아래 `.hint` 한 줄. 판단 기준은 아래 "부피 등급(`vol`)과 가방 추천" 참고
- **상비 항목** 완료 — 항목마다 "상비" 칩으로 토글(`S.stock`). 상비 항목은 흐리게 표시되고 진행 게이지에서 자동으로 "완료"로 집계된다(매번 체크 안 해도 됨). 조건과 무관한 개인 습관이라 임무를 불러와도 초기화되지 않는다. 가방 부피 계산엔 그대로 포함됨(실제로 가방 안에 있는 물건이므로)

### 로드맵 완료 — 다음은 자유 기능 추가
CLAUDE.md 초안의 순차 로드맵(배포→인증→저장→공개·별점)은 여기서 끝났다. 이제부터는 "아직 정하지 않은 것"에서 우선순위를 정해 진행한다.

---

## 확정된 결정 (바꾸지 말 것 / 바꾸려면 먼저 논의)

### 이름과 용어

| 개념 | 쓰는 말 | 쓰지 않는 말 |
|---|---|---|
| 외출 | **외출** | 나들이 (실제 육아 콘텐츠에서 거의 안 쓴다) |
| 프리셋 킷 | **임무** | 템플릿, 킷 |
| 체크리스트 항목 | **보급품** (헤더에서만) | — |
| 아이 | **대장님** (완료 메시지에서만) | 키즈 (유치원~초등을 가리켜 대상이 어긋난다) |
| 사용자 | **부모** | 맘, 엄마 (아빠 사용자를 배제한다) |

### 문구 규칙 — 중요

**은유는 감정이 오가는 자리에만, 기능 라벨은 평범한 말로.**

- 은유 O: 헤더, 완료 메시지, 빈 화면, 임무 이름, 섹션 제목
- 은유 X: 아이템 이름, 폼 라벨, 수량, 카테고리명

"기저귀"를 "보급품 3호"라고 부르면 안 된다. 급하게 쓰는 앱이라 찾기 어려워지는 순간 실패한다.
**은유는 아껴 쓸수록 세게 박힌다.** 현재 "대장님"은 완료 메시지 딱 한 곳에만 있다. 늘리지 말 것.

### 기술 선택

- **단일 HTML 파일.** 빌드 도구, 프레임워크, npm 없음. 프로토타입 단계에서는 이걸 유지한다.
- **`Store` 어댑터 패턴.** localStorage ↔ Supabase 전환이 이 객체 한 곳에서 끝난다. 이 경계를 무너뜨리지 말 것.
- **규칙 엔진은 `ITEMS` 배열.** 각 항목이 `{id, c(카테고리), n(이름), when(ctx), q(ctx), why(ctx), warn, vol}`.
  카테고리 트리를 만들지 말고 조건 함수를 붙이는 방식을 유지할 것.
- **수량은 반드시 구조화된 값으로.** `q(ctx)`는 `{value, unit}`을 반환한다 (`value:null`이면 수량 없이 단위만, 예: 파우치).
  **표시용 문자열("5장")을 반환하거나 저장하지 말 것** — 재계산·수정·합산이 막히고, 그건 이 앱의 존재 이유를 깨뜨린다.
  화면 표시는 `fmtQty()` 하나만 담당한다.
- **강조 표시는 `warn:true` 플래그로.** 예전엔 why 텍스트를 정규식으로 검사했는데, 문구만 고쳐도 스타일이 조용히 깨졌다. 되돌리지 말 것.
- **Supabase Free 플랜.** 7일간 DB 요청이 없으면 자동 일시정지된다. 데모 전에 한 번 깨울 것.

### 부피 등급(`vol`)과 가방 추천

리터 단위 정밀 계산은 **하지 않는다.** 제품마다 달라서 거짓 정확도가 되고, 그건 신뢰를 깎는다.
대신 항목마다 등급을 매기고 합계를 문구로 바꾼다. **새 항목을 추가할 때도 반드시 아래 앵커와 상대비교로 판단할 것** ("이건 기저귀보단 크고 여벌옷보단 작으니 3").

- **척도는 `1, 2, 3, 5, 8`** (피보나치형). 간격을 넓혀 애매한 구간을 없앤 것이니 1~10 같은 촘촘한 척도로 바꾸지 말 것.
- **앵커 (바꾸지 말 것 — 모든 판단의 기준점)**

  | vol | 기준 항목 |
  |---|---|
  | 1 | 기저귀 1장(`diaper`), 밴드·소독솜 파우치(`kit`) |
  | 2 | 지갑·카드·차키(`wallet`), 젖병 1개(`bottle`) |
  | 3 | 여벌 양말(`sock`), 간식 1봉(`snack`) |
  | 5 | 여벌 옷 1벌(`spare`) |
  | 8 | 담요/속싸개(`blanket`) |

- **수량이 있는 항목은 낱개·1회분 기준으로 매긴다.** 합산 로직(`bagVolume`)이 수량을 곱한다.
- **가방 "안"에 안 들어가는 것은 `vol`을 아예 넣지 않는다.** 현재 `carrier`(착용) · `stroller`(이동수단 자체) · `carseat`(차에 둠) 3개. 합계에서 0으로 빠진다.
- **"항목별 등급"과 "합계→문구 매핑"은 분리돼 있다.** 기준선이나 추천 문구만 고치고 싶으면 `BAG_TIERS` 배열 하나만 손대면 된다 — `ITEMS`를 재조정하지 말 것.
  현재 기준선(30 / 55 / 90)은 실제 조합을 돌려 잡았다: 만 3세·짧은 외출 ≈ 40, 12개월·3시간 표준 ≈ 73, 한겨울 8시간 최대치 ≈ 110.
- 합계는 **체크 여부와 무관하게 조건에 맞는 전체 항목**으로 낸다. 가방은 짐을 싸기 전에 정하는 것이라서다.
- 표시는 게이지 아래 `.hint` 한 줄. **시그니처는 게이지 하나뿐이므로 여기에 색·강조를 붙이지 말 것.**

### 디자인 토큰 (index.html의 `:root`)

```
--paper #EDF0EC   배경
--ink   #17231C   본문
--moss  #2F6B4F   주색 (딥 그린)
--sprout #7FB069  게이지 그라데이션
--amber #D98E2B   주의 표시
```

- 본문/UI: Pretendard / 숫자·라틴 유틸리티 라벨: Space Grotesk
- 시그니처 요소는 **상단 sticky 진행 게이지**다. 여기가 유일하게 화려한 곳이고 나머지는 조용해야 한다.

---

## 보안 — 반드시 지킬 것

- **`sb_publishable_...` 키는 커밋해도 된다.** 브라우저 코드라 어차피 공개된다. 숨기려 애쓰지 말 것.
  대신 **RLS가 유일한 방어선**이므로 절대 끄지 않는다.
- **절대 커밋 금지:** `sb_secret_...` 키, 데이터베이스 비밀번호. 프론트엔드에 들어갈 일이 아예 없다.
- 이 프로젝트는 **"Automatically expose new tables"를 꺼둔 상태**로 생성됐다.
  → **새 테이블을 만들면 `schema.sql`의 GRANT 블록에 추가해야 Data API에 노출된다.** 빠뜨리면 "relation does not exist" 류 에러가 난다.
- "Enable automatic RLS"는 켜져 있어 새 테이블에 RLS가 자동으로 걸린다. 정책은 여전히 직접 써야 한다.

---

---

## 앱(안드로이드·iOS) 전환 — 언제, 어떻게

**결론: 지금은 웹(PWA)으로 간다. 네이티브는 재작성이 아니라 지금 코드를 감싸는 것이다.**
React Native·Flutter 재작성은 하지 말 것 — 이 앱은 폼+계산+체크리스트라 네이티브 성능이 필요 없고, 코드베이스만 둘로 늘어난다.

수익화 방식이 경로를 가른다:

| | 웹(PWA) 유지 | Capacitor로 감싸기 |
|---|---|---|
| 광고 | AdSense만. 체류가 짧아 수익 미미 | **AdMob 가능** (웹에선 불가) |
| 결제 | 토스·카카오페이 → **수수료 0%** | 인앱결제 강제 → **15~30%** |
| iOS | 홈 화면 추가로 사용 | 단순 웹뷰는 심사 거절 위험 |

- 안드로이드 스토어만 원하면 **TWA**(Bubblewrap)가 중간 단계다. 실제 https 주소를 그대로 쓰므로 **구글 로그인이 수정 없이 동작**한다.
- Capacitor로 가면 `signInWithOAuth`의 `redirectTo`를 딥링크로 재작업해야 한다. Mac/Xcode도 필요해진다.

### "단일 HTML 파일" 결정을 바꿀 트리거

아래 신호가 보이기 전엔 바꾸지 말 것 (조기 도입은 순손실):

- **JS 1,500~2,000줄 초과** 또는 **협업자 2인** 또는 파일 안에서 길찾기 실패 → 파일 분할(빌드 도입)
- **수량 인라인 편집 도입** 또는 화면 항목 상시 150개 이상 → 전체 innerHTML 재생성을 타깃 업데이트로
- **아이 여러 명 / 날씨 자동 채움 착수** → 중앙 `setState(patch)` 함수 하나 (Redux 아님)
- **AdMob 또는 iOS 앱스토어 확정** → Capacitor. 이때 처음으로 npm/빌드가 정당화된다

---

## 아직 정하지 않은 것

- **입력 항목이 6개인데 많다.** 실사용해보고 줄일 것. 기온은 날씨 API(기상청 단기예보)로 자동 채우면 하나 줄어든다.
- 광고·결제 중 무엇을 주력으로 할지 (위 표 참고 — 이걸 정해야 앱 전환 여부가 정해진다)
- 아이 여러 명 지원 — 현재 한 명 기준
- 도메인, 앱스토어 이름 중복 확인 미완료

## 파일

```
index.html     앱 전체 (단일 파일)
manifest.json  PWA 매니페스트. start_url·scope는 상대경로("./") — 하위 경로 배포라 절대경로면 깨진다
sw.js          서비스워커. 앱 셸 network-first / CDN cache-first / *.supabase.co는 절대 캐시 금지
icons/         앱 아이콘 (PNG 4개 + SVG 2개)
schema.sql     Supabase 스키마. 적용 완료. 구조 참고용으로 남겨둠
docs/          배포 가이드 HTML 2개. .gitignore로 커밋 제외됨 (로컬 참고용)
CLAUDE.md      이 파일
```
